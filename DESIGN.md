...
### Logo Lockup
Never hand-built or recreated in HTML/CSS — the brand mark is locked. Every instance renders from the official exports in `_design-system/brand-assets/` (mirrored into `public/assets/`), not a live-scaling component. The full lockup is the default everywhere it clears the asset manifest's own floor — "compact when the full lockup would fall under 120px wide" (`logo-compact.card.html`, `logo-mark-scale.card.html`) — and only drops to compact where a breakpoint would otherwise shrink it under that:
- **Header**: `logo-primary.svg` at 187px wide (well past the 120px floor) above 860px, swapping to `logo-compact.svg` (mark + gold rule + wordmark, no descriptor/principal) below it via `<picture>` — the same breakpoint where the inline nav collapses. Mobile menu bar uses `logo-compact-reversed.svg` at 40px mark height.
- **Legal header**: the same `logo-primary.svg` / `logo-compact.svg` swap, at its own 620px breakpoint (no nav to key off here).
...
- Every lockup in use is SVG, not PNG: the asset package's lockup SVGs carry outlined text (no `font-family` dependency), so they're safe as an `<img src>` — crisp at any zoom, a fraction of the PNG weight. An earlier package version set live text instead, which broke when loaded this way (an `<img>` can't see the host page's fonts, so it silently falls back to a wider system serif and clips against the fixed `viewBox` — confirmed: `logo-compact.svg` clipped "NTA, Inc." to "NTA, In"); fixed upstream in the current asset package.
